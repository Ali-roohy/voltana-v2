...
| TASK-0018 | developer | **BACKLOG** — **Phase-3 feature** (pm spec 2026-06-03). Odometer tracking for efficiency: optional `odometer_km` on the charging-session form → `charging_sessions.odometer_km` (column **already exists** — verify); per-session **kWh/100km** from two consecutive readings (`km_driven = odo_new − odo_old`, guard >0); show on session card + revive the dashboard avg-efficiency (real data, not "—") via the analytics service. No new external dep. Reviewer: dev_supervisor (no security — no auth surface). **Flags:** confirm the column + the post-0011 dashboard avg-efficiency wiring. **Depends on TASK-0004 + TASK-0008 (✅ CLOSED).** |
