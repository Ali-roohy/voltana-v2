...
| TASK-0042 | developer | **IN_PROGRESS** (2026-06-24) — Analytics Bugs + Smart Session Features. **Done & pushed:** BUG-1 (Jalali axes), BUG-2/3 (expanded list, odometer order), BUG-4 (cumulative-odometer + 5–40 efficiency guard, live: avg 47.55→24.86), BUG-5 (unclamp SOH, live 100→106.14; migration 000020), migration 000021 (trip_distance_km/charge_power_kw/city/regen_factor + backfill), FEAT-3 (charge power + location memory), FEAT-5 **backend** (warnings[], live 17/56 flagged), FEAT-1 (smart SOC via `estimateConsumption(context)`), FEAT-6 (optional+predicted duration), FEAT-4 (regen factor + elevation note). **Deferred to ONE Fable design pass:** FEAT-5 frontend (red warning rows/tooltip/entry-time) + FEAT-2 (seasonal expected-consumption band). `estimateConsumption` season slot reserved. See `.ai/workflows/TASK-0042.md`. |
