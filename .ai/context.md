...
- **Date**: 2026-06-23
- **TASK-0042 — Analytics Bugs + Smart Session Features — READY** (created 2026-06-23, pm). Born from
  a qa verification of the admin's **56 real charging sessions** vs. an external reference (2026-06-23).
  **QA verdict:** cost path **fully correct** (0/56 mismatches, rates snapshotted, peak+mid+off ==
  kwh_charged on every row), recommendations correct (LFP→100), edge-case exclusions correct.
  **Two real defects + 3 UX bugs + 6 features** bundled:
  - **BUG-4 (HIGH):** `odometer_km` entered as a **cumulative** odometer (operator-confirmed) but
    bounces 174→401 with **no monotonic guard** → `total_km=1022`, `avg=47.55 kWh/100km` (per-session
    up to 483). Fix: reject decreasing odometer, store `trip_distance_km`, derive trip from SOC×range
    when odometer absent, 5–40 kWh/100km sanity guard on the fleet avg.
  - **BUG-5 (MEDIUM):** SOH clamped to 100% at `analytics_service.go:287-289` hides the real estimate
    (≈106% / 50.4 kWh on this car). Fix: unclamp, allow >100% + "باتری سالم‌تر از حد انتظار" badge.
    **Operator flag (not autonomous):** catalog nominal for Dongfeng E70 = 47.5 kWh but data implies
    52–55 kWh usable — operator to correct catalog if desired.
  - **BUG-1 (CRITICAL fa):** Jalali chart axes via centralized `src/lib/dates.ts` (date-fns +
    date-fns-jalali already in package.json); **BUG-2:** sessions expanded by default; **BUG-3:**
    odometer field after off-peak energy.
  - **Section B features:** migration 000020 (`trip_distance_km`, `charge_power_kw`,
    `user_settings.city`, `user_settings.regen_factor` + backfill); FEAT-1 smart SOC, FEAT-2 seasonal
    expected-consumption band, FEAT-3 charge-power + location memory, FEAT-4 regen/elevation, FEAT-5
    suspicious-data warnings, FEAT-6 optional+predicted duration. **Architecture requirement:** a single
    extensible `estimateConsumption(context)` (season + regen plug in, no forks); all dates via
    `lib/dates.ts`. Sequence: BUG-1→5 → migration → FEAT-3 → FEAT-5 → FEAT-1 → FEAT-6 → FEAT-2 → FEAT-4.
    Design passes (seasonal band, warning styling) switch to Fable, implement on Opus.
  - **Next:** kick off the chain on **BUG-1** (developer).
- **Dashboard note:** `voltana-dashboard.html` TASKS array only held cards through TASK-0025 (0026–0041
  were never added as cards); stat **counts** are recomputed from all workflow files so headlines are
  correct (41 tasks, DONE:40, READY:1). Added a TASK-0042 card. Full card backfill for 0026–0041 is a
  separate cleanup, out of scope here.
- (prior) **Date**: 2026-06-12
...
| TASK-0042 | developer | **DONE ✅ CLOSED with caveat** (2026-06-24) — Analytics Bugs + Smart Session Features. 12 slices: BUG-1 (Jalali axes via `lib/dates.ts`), BUG-2/3 (expanded list, odometer order), BUG-4 (cumulative-odometer 422 + 5–40 efficiency guard, live avg 47.55→24.86), BUG-5 (unclamp SOH, live 100→106.14; migration 000020), migration 000021 (trip_distance_km/charge_power_kw/city/regen_factor + backfill), FEAT-3 (charge power + location memory), FEAT-5 (warnings[] backend live 17/56 + Fable-designed amber list/entry styling), FEAT-1 (smart SOC), FEAT-6 (optional+predicted duration), FEAT-4 (regen + elevation note), FEAT-2 (seasonal band on monthly chart, Tir–Shahrivar ×1.15 / Dey–Esfand ×1.20). **Architecture rule honored:** single `estimateConsumption(context)` — car avg + regen + season all layer in one function (`features/analytics/season.ts` shared by chart + estimator). Fable design pass recorded in the task file. ⚠️ Chart/form visuals retire with operator browser pass (no host browser). **Operator decision open:** E70 catalog capacity 47.5 vs ~52–55 kWh. |
